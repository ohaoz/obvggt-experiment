...
def train(args):
    ...
    if accelerator.is_main_process:
        swanlab_state = init_swanlab_run(
            config=args,
            job_type="finetune",
            default_dataset="mixed_train",
            output_dir=args.output_dir,
            config_for_log=OmegaConf.to_container(args, resolve=True),
        )
    ...
    try:
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            ...
            if accelerator.is_main_process:
                epoch_metrics = {f"finetune/{k}": v for k, v in train_stats.items()}
                epoch_metrics["epoch"] = float(epoch)
                log_swanlab(swanlab_state, epoch_metrics, step=int(epoch * 1000))
