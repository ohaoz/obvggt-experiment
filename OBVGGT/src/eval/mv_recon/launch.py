...
def main(args):
    ...
    if accelerator.is_main_process:
        swanlab_state = init_swanlab_run(
            config=args,
            job_type="eval_mv_recon",
            default_dataset="7scenes+NRGBD",
    ...
    if kv_cache_cfg is not None and model_name != "StreamVGGT":
        raise ValueError("mv_recon KV cache evaluation is only supported for StreamVGGT.")

    effective_kv_cache_cfg = kv_cache_cfg if model_name == "StreamVGGT" else None
    del ckpt
    ...
    with torch.no_grad():
        for name_data, dataset in datasets_all.items():
            ...
            kv_meta = write_kv_metadata(
                save_path,
                effective_kv_cache_cfg,
                task="mv_recon",
                benchmark_role="streaming_benchmark",
                extra={"eval_dataset": name_data, "model_name": model_name},
            )
            ...
            with accelerator.split_between_processes(list(range(len(dataset)))) as idxs:
                for data_idx in tqdm(idxs):
                    ...
                    if model_name in {"StreamVGGT", "VGGT"}:
                        ...
                        with torch.cuda.amp.autocast(dtype=dtype):
                            with torch.no_grad():
                                if model_name == "StreamVGGT":
                                    results = model.inference(batch, kv_cache_cfg=effective_kv_cache_cfg)
                                else:
                    ...
                    latency_ms_per_frame = float(1000.0 * elapsed_sec / num_frames) if num_frames > 0 else 0.0
                    scene_stats = {
                        "scene_id": scene_id,
                        "status": "ok",
                    ...
                    scene_stats.update({key: value for key, value in kv_meta.items() if key != "kv_cfg"})
                    if isinstance(kv_stats, dict):
                        for key, value in kv_stats.items():
                            try:
                                scene_stats[f"kv_{key}"] = float(value)
                            except (TypeError, ValueError):
                                continue
                    with open(system_log_path, "a", encoding="utf-8") as f_sys:
                        f_sys.write(json.dumps(scene_stats, ensure_ascii=False) + "\n")
            ...
            if accelerator.is_main_process:
                ...
                with open(summary_path, "w", encoding="utf-8") as summary_file:
                    summary_payload = dict(mean_metrics)
                    summary_payload["_meta"] = kv_meta
                ...
                with open(system_metrics_path, "w", encoding="utf-8") as system_file:
                    json.dump(
                        {"summary": system_summary, "per_scene": merged_records, "_meta": kv_meta},
                        system_file,
                        ensure_ascii=False,
                        indent=2,
                    )
                swanlab_metrics = {
                    f"mv_recon/{name_data}/{metric_name}": float(metric_value)
