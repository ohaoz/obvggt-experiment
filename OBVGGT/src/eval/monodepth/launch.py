...
def eval_mono_depth_estimation(args, model, device, kv_cache_cfg=None):
    print("[monodepth] benchmark_role=regression_only; do not use this task as the primary KV-effectiveness benchmark.")
...
def eval_mono_depth(args, model, device, filelist, save_dir=None, kv_cache_cfg=None):
    model.eval()
    load_img_size = 518
    for file in tqdm(filelist):
        ...
        if save_dir is not None:
            file_stem = Path(file[0]).stem
            # save the depth map to the save_dir as npy
            np.save(
                f"{save_dir}/{file_stem}_depth.npy",
            ...
            cv2.imwrite(
                f"{save_dir}/{file_stem}_depth.png",
                depth_map,
            )
...
if __name__ == "__main__":
    ...
    eval_mono_depth_estimation(args, model, args.device, kv_cache_cfg=kv_cache_cfg)
