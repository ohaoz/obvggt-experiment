...
from swanlab_utils import finish_swanlab, init_swanlab_run, log_swanlab
...
def setup_args():
    ...
    parser.add_argument('--fast_eval', action='store_true', help='Evaluate a subset of sequences for quick validation')
    ...
    parser.add_argument("--swanlab_project", type=str, default="StreamVGGT")
    parser.add_argument("--swanlab_workspace", type=str, default="")
    parser.add_argument("--swanlab_experiment_name", type=str, default="")
    parser.add_argument("--swanlab_tags", type=str, default="")
    parser.add_argument("--swanlab_logdir", type=str, default="")
    parser.add_argument("--swanlab_api_key_env", type=str, default="SWANLAB_API_KEY")
...
def process_sequence(
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast(dtype=dtype):
            output = model.inference(batch, kv_cache_cfg=kv_cache_cfg)
            predictions = output.ress
            kv_stats = getattr(output, "kv_cache_stats", {}) or {}
    ...
    with torch.cuda.amp.autocast(dtype=torch.float64):
        ...
        return rel_rangle_deg.cpu().numpy(), rel_tangle_deg.cpu().numpy(), kv_stats
...
def main():
    ...
    kv_meta = write_kv_metadata(
        ...
        kv_cache_cfg,
        task="pose_co3d",
        benchmark_role="optional_streaming_benchmark",
        extra={"eval_dataset": "co3d"},
    )
    swanlab_state = init_swanlab_run(
        config=args,
        job_type="eval_pose_co3d",
        default_dataset="co3d",
        output_dir=args.output_dir,
        config_for_log=vars(args),
    )
    ...
    for category in SEEN_CATEGORIES:
        ...
        for seq_name in seq_names:
            ...
            seq_rError, seq_tError, seq_kv_stats = process_sequence(
            ...
            if seq_rError is not None and seq_tError is not None:
                ...
                record = {"category": category, "sequence": seq_name}
                record.update({key: value for key, value in kv_meta.items() if key != "kv_cfg"})
                if isinstance(seq_kv_stats, dict):
                    for key, value in seq_kv_stats.items():
                        try:
                            record[f"kv_{key}"] = float(value)
        ...
        category_metrics = {
            f"pose_co3d/{category}/Auc_30": float(Auc_30),
            f"pose_co3d/{category}/Auc_15": float(Auc_15),
            f"pose_co3d/{category}/Auc_5": float(Auc_5),
            f"pose_co3d/{category}/Auc_3": float(Auc_3),
        }
        log_swanlab(swanlab_state, category_metrics)
    ...
    if per_category_results:
        ...
        summary["_meta"] = kv_meta
        if kv_records:
            kv_evicted_total = float(sum(float(record.get("kv_evicted_tokens_total", 0.0)) for record in kv_records))
            kv_evict_calls_total = float(sum(float(record.get("kv_evict_calls", 0.0)) for record in kv_records))
            kv_appended_total = float(sum(float(record.get("kv_appended_tokens_total", 0.0)) for record in kv_records))
            kv_reused_total = float(sum(float(record.get("kv_reused_tokens_total", 0.0)) for record in kv_records))
            kv_denom = kv_appended_total + kv_reused_total
            summary["_system"] = {
                "num_sequences_ok": int(len(kv_records)),
        ...
        summary_path = os.path.join(args.output_dir, "pose_summary.json")
        with open(summary_path, "w", encoding="utf-8") as summary_file:
            json.dump(summary, summary_file, ensure_ascii=False, indent=2)
        system_metrics_path = os.path.join(args.output_dir, "system_metrics.json")
        with open(system_metrics_path, "w", encoding="utf-8") as system_file:
            json.dump(
                {"summary": summary.get("_system", {}), "per_sequence": kv_records, "_meta": kv_meta},
                system_file,
                ensure_ascii=False,
                indent=2,
            )
        summary_metrics = {
            "pose_co3d/mean/Auc_30": float(mean_AUC_30),
            "pose_co3d/mean/Auc_15": float(mean_AUC_15),
            "pose_co3d/mean/Auc_5": float(mean_AUC_5),
            "pose_co3d/mean/Auc_3": float(mean_AUC_3),
        }
        log_swanlab(swanlab_state, summary_metrics)

    finish_swanlab(swanlab_state)
