...
def main(args):
    if args.eval_dataset == "nyu":
        ...
        for p in sorted(glob.glob(f"{args.output_dir}/*/*.npy")):
            ...
            # Handle both 'frame_0000.jpg.npy' and 'frame_0000depth.npy'
            frame = fname.replace('.jpg.npy', '').replace('depth.npy', '').replace('.npy', '')
        ...
        for d in depth_pathes_raw:
            ...
            pred_path = pred_lookup.get((scene, frame))
            if pred_path:
                depth_pathes.append(d)
                pred_pathes.append(pred_path)
        print(
            f"scannet: matched {len(depth_pathes)} pairs "
